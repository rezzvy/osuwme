class Normalizer {
  ...
  _pushDownProperties(element) {
    ...
    const isSpanWrapperForMergeable = isSpan && hasSingleChild && this.config.tagsToMerge.includes(childTag);

    if (element.style && element.style.length > 0) {
      for (let i = 0; i < element.style.length; i++) {
        const prop = element.style[i];
        if (this.config.stylesToPushDown.includes(prop) || isSpanWrapperForMergeable) {
          stylesToMove.push({
            prop: prop,
  ...
  _hoistCommonStyles(parent) {
    ...
    const allowedTargets = ["SPAN", "A", "CODE", "STRONG", "EM", "B", "I", "U", "S"];
    if (!allowedTargets.includes(parent.tagName)) {
